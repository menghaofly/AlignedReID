...
import models
from util.losses import CrossEntropyLoss, DeepSupervision, CrossEntropyLabelSmooth, AngleLoss
...
def test(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    # print('origin shape: ', distmat.shape)
    # distmat = torch.mm(torch.Tensor(qf), torch.Tensor(gf).t())
    # distmat = torch.Tensor(1) - distmat
    # print('new shape: ', distmat.shape)
...
def main():
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'a_softmax'}, use_gpu=use_gpu)
